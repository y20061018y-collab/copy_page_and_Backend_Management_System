...
type Dashboard = { game_count: number; active_game_count: number; service_count: number; active_service_count: number; content_health_score: number; latest_updated_at: string | null };
...
export default function AdminDashboard() {
  ...
  const healthScore = dashboard?.content_health_score;
  const healthRingStyle = { "--admin-health-score": `${healthScore ?? 0}%` } as CSSProperties;
  const healthStatus = healthScore == null ? "内容状态加载中" : healthScore >= 90 ? "内容状态良好" : healthScore >= 70 ? "内容待完善" : "内容需要补充";
  ...
  return <AdminLayout active="dashboard" crumb="控制台">
    ...
    <section className="admin-v2-stats">
      ...
      <article className="admin-v2-stat"><div><span>服务</span><i>☷</i></div><strong>{dashboard?.service_count ?? "–"}</strong><small>当前前台展示 {dashboard?.active_service_count ?? "–"} 项</small></article>
    ...
    <section className="admin-v2-grid">
      ...
      <article className="admin-v2-panel"><div className="admin-v2-panel-head"><h2>内容健康度</h2><span>前台资料完整性</span></div><div className="admin-v2-health">
        <div className="admin-v2-score"><div className="admin-v2-ring" style={healthRingStyle}><b>{healthScore ?? "–"}</b></div><div><strong>{healthStatus}</strong><p>游戏、服务和站点资料按当前完整性计算。</p></div></div>
    ...
    <section className="admin-v2-grid">
      ...
      <article className="admin-v2-panel"><div className="admin-v2-panel-head"><h2>最近更新</h2><span>按数据更新时间汇总</span></div><div className="admin-v2-activity">
        <div className="admin-v2-activity-row"><i>CAT</i><div><b>游戏目录与服务</b><small>{dashboard ? `${dashboard.game_count} 个游戏，${dashboard.service_count} 项服务` : "加载中"}</small></div><time>实时</time></div>
