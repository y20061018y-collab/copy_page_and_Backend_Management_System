...
export default function AdminServices() {
  ...
  return <AdminLayout active="services" crumb="服务"><AdminPageTitle eyebrow="SERVICE CATALOG" title="服务" description="按游戏维护服务说明、启停状态与前台展示顺序。" />
    ...
    <section className="admin-v2-panel"><table className="admin-v2-table"><thead><tr><th>服务</th><th>所属游戏</th><th>服务说明</th><th>状态</th><th>编辑</th></tr></thead><tbody>{services.map((service) => <tr key={service.id}><td><b>{service.name}</b></td><td><div className="admin-v2-game-cell"><img src={service.game.cover_image} alt=""/><b>{service.game.name}</b></div></td><td>{service.description || "暂未填写服务说明"}</td><td><span className={`admin-v2-status ${service.is_active ? "" : "is-off"}`}>{service.is_active ? "展示中" : "已隐藏"}</span></td><td><a className="admin-v2-secondary" href={`/admin/games/${service.game.id}`}>编辑</a></td></tr>)}{!services.length && <tr><td colSpan={5}>暂未添加服务</td></tr>}</tbody></table></section>
  </AdminLayout>;
}
